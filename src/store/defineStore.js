...
export function defineStore (name, options) {
  ...
  return function useStore (ctx, isUnRegister = true) {
    ...
    return {
      ...
      $getters: function (getterName) {
        return Store.getters[name + '/' + getterName]
      },
      commit (mutation, payload) {
        return Store.commit(name + '/' + mutation, payload)
      },
      $commit: Object.keys(Store._mutations)
        .filter(key => key.startsWith(name))
      ...
      dispatch (action, payload) {
        return Store.dispatch(name + '/' + action, payload)
      },
      $dispatch: Object.keys(Store._actions)
        .filter(key => key.startsWith(name))
        .map(mut => ({ rowName: mut, name: mut.split('/')[1] }))
        .reduce((pre, cur) => {
          return {
